...
bool BoundingBox::imp_intersects(BoundingBox &b) {
  Vertex minmax[] = {b.m_min, b.m_max};
  for (unsigned int xyz = 0; xyz < 8; ++xyz)
    if ( (m_max.m_x >= minmax[xyz & 0x1].m_x &&
	  m_max.m_y >= minmax[(xyz >> 1) & 0x1].m_y &&
	  m_max.m_z >= minmax[(xyz >> 2) & 0x1].m_z) &&
	 (m_min.m_x <= minmax[xyz & 0x1].m_x &&
	  m_min.m_y <= minmax[(xyz >> 1) & 0x1].m_y &&
	  m_min.m_z <= minmax[(xyz >> 2) & 0x1].m_z) ||
	 ((m_max.m_x >= minmax[xyz & 0x1].m_x && m_max.m_y >= minmax[(xyz >> 1) & 0x1].m_y &&
	   m_max.m_z <= minmax[1].m_z && m_min.m_z >= minmax[0].m_z) &&
	  (m_min.m_x <= minmax[xyz & 0x1].m_x && m_min.m_y <= minmax[(xyz >> 1) & 0x1].m_y &&
	   m_max.m_z <= minmax[1].m_z && m_min.m_z >= minmax[0].m_z)) ) 
      return true;
  return false;
}
...
void BoundingBox::drawBBox() {
  // Ugly... but works!
  Vertex &m_bBoxMax = m_max;
  Vertex &m_bBoxMin = m_min;
  
  glBegin(GL_LINES);
  // Top
  glColor3f(1,1,0);
  glVertex3f(m_bBoxMax.m_x, m_bBoxMax.m_y, m_bBoxMax.m_z);
  glVertex3f(m_bBoxMax.m_x, m_bBoxMax.m_y, m_bBoxMin.m_z);
  
  glVertex3f(m_bBoxMax.m_x, m_bBoxMax.m_y, m_bBoxMin.m_z);
  glVertex3f(m_bBoxMin.m_x, m_bBoxMax.m_y, m_bBoxMin.m_z);
  
  glVertex3f(m_bBoxMin.m_x, m_bBoxMax.m_y, m_bBoxMin.m_z);
  glVertex3f(m_bBoxMin.m_x, m_bBoxMax.m_y, m_bBoxMax.m_z);
  
  glVertex3f(m_bBoxMin.m_x, m_bBoxMax.m_y, m_bBoxMax.m_z);
  glVertex3f(m_bBoxMax.m_x, m_bBoxMax.m_y, m_bBoxMax.m_z);
  
  // Bottom
  glVertex3f(m_bBoxMin.m_x, m_bBoxMin.m_y, m_bBoxMax.m_z);
  glVertex3f(m_bBoxMin.m_x, m_bBoxMin.m_y, m_bBoxMin.m_z);
  
  glVertex3f(m_bBoxMin.m_x, m_bBoxMin.m_y, m_bBoxMin.m_z);
  glVertex3f(m_bBoxMax.m_x, m_bBoxMin.m_y, m_bBoxMin.m_z);
  
  glVertex3f(m_bBoxMax.m_x, m_bBoxMin.m_y, m_bBoxMin.m_z);
  glVertex3f(m_bBoxMax.m_x, m_bBoxMin.m_y, m_bBoxMax.m_z);
...
void BoundingBox::addToBBox(const Vertex &v) {
  if (m_min.m_x == UNDEFINED_VALUE)
    m_min = v;
  if (m_max.m_x == UNDEFINED_VALUE)
    m_max = v;
  // + SMALL_DBL because the bounding box shall be slightly bigger.
  m_max.m_x = (v.m_x != UNDEFINED_VALUE) ? max(m_max.m_x, v.m_x + SMALL_DBL) : m_max.m_x;
  m_max.m_y = (v.m_y != UNDEFINED_VALUE) ? max(m_max.m_y, v.m_y + SMALL_DBL) : m_max.m_y;
  m_max.m_z = (v.m_z != UNDEFINED_VALUE) ? max(m_max.m_z, v.m_z + SMALL_DBL) : m_max.m_z;
    m_min.m_x = (v.m_x != UNDEFINED_VALUE) ? min(m_min.m_x, v.m_x - SMALL_DBL) : m_min.m_x;
    m_min.m_y = (v.m_y != UNDEFINED_VALUE) ? min(m_min.m_y, v.m_y - SMALL_DBL) : m_min.m_y;
    m_min.m_z = (v.m_z != UNDEFINED_VALUE) ? min(m_min.m_z, v.m_z - SMALL_DBL) : m_min.m_z;
}
...
ConcreteShape::ConcreteShape(float *color, bool isPiece) : m_boundingBox(), m_isPiece(isPiece) {
	transformation tmp = {{0.f,0.f,0.f},{1.f,0.f,0.f,0.f}, {.05f,.05f,.05f}};
	m_tra = tmp;
	memcpy(m_color, color, sizeof(m_color));
}

ConcreteShape::ConcreteShape(float *translation, float *rotation, float *scale, float *color, bool isPiece) {
...
void ConcreteShape::draw() {
	glCallList(m_list);
}

transformation* ConcreteShape::getTrans(){
	return &m_tra;
}

float * ConcreteShape::getColor(){
	return m_color;
}

bool ConcreteShape::isPiece() {
	return m_isPiece;
}
...
StraightBox::StraightBox(Vertex min, Vertex max, float *c, bool isPiece):
  ...
  if (isPiece)
    m_zOffset = SMALL_DBL;
  m_boundingBox.addToBBox(min);
  m_boundingBox.addToBBox(max);
}
...
GayBox::GayBox(Vertex b1, Vertex b2,double width, int alignment, float *c, bool isPiece) 
  : ConcreteShape(c, isPiece), m_back1(b1), m_back2(b2), m_width(width), m_alignment(alignment), m_zoffset(0) {
...
void GayBox::create(){
  ...
  // Add candidates to the bounding box.
  m_boundingBox.addToBBox(m_back1.m_x+m_width/2, m_back1.m_y+m_width/2, m_back1.m_z);
  m_boundingBox.addToBBox(m_back1.m_x-m_width/2, m_back1.m_y-m_width/2, -m_zoffset);
  m_boundingBox.addToBBox(m_back2.m_x+m_width/2, m_back2.m_y+m_width/2, m_back2.m_z);
  m_boundingBox.addToBBox(m_back2.m_x-m_width/2, m_back2.m_y-m_width/2, -m_zoffset);
}
...
Cylinder::Cylinder(Vertex bottomCenter, GLfloat r, GLfloat h, GLfloat *c, bool isPiece) :
  Quadric(c, isPiece), m_bottomCenter(bottomCenter), m_r(r), m_h(h) { 
  static int cnt = 0;
#ifdef _DEBUG_MODE_
...
void Cylinder::create() {
  ...
  for(int i=0 ; i <= 360 ; i += 12){
    ...
    glVertex3f(CosSinTable::getInstance().sin(i)*m_r,CosSinTable::getInstance().cos(i)*m_r,0);
    ColorSetter::getInstance().setColor(m_bottomCenter.m_z + m_h);
    glVertex3f(CosSinTable::getInstance().sin(i)*m_r,CosSinTable::getInstance().cos(i)*m_r, m_h);
  }
  glEnd();
  glPopMatrix();
  glPushMatrix();
  ...
  // Add max and minpoints to bounding box.
  m_boundingBox.addToBBox(m_bottomCenter.m_x - m_r, m_bottomCenter.m_y - m_r, m_bottomCenter.m_z);
  m_boundingBox.addToBBox(m_bottomCenter.m_x + m_r, m_bottomCenter.m_y + m_r, m_bottomCenter.m_z + m_h);
}
...
void SolidArc::create() {
  ...
  glNewList(m_list, GL_COMPILE);
	...
	// Add candidates to bounding box.
	if (m_clockwise) {
	  Vertex v(CosSinTable::getInstance().sin(i)*m_innerRadius + m_bottomCenter.m_x,
		   CosSinTable::getInstance().cos(i)*m_innerRadius + m_bottomCenter.m_y,
		   m_bottomCenter.m_z);
	  m_boundingBox.addToBBox(v);
	}
	else {
	  Vertex v(CosSinTable::getInstance().cos(i-90)*m_innerRadius + m_bottomCenter.m_x,
		   CosSinTable::getInstance().sin(i-90)*m_innerRadius + m_bottomCenter.m_y,
		   m_bottomCenter.m_z);
	...
	// Add a candidate to the boundry box.
	if (m_clockwise) {
	  Vertex v(CosSinTable::getInstance().sin(i)*m_outerRadius + m_bottomCenter.m_x,
		   CosSinTable::getInstance().cos(i)*m_outerRadius + m_bottomCenter.m_y,
		   m_height + m_bottomCenter.m_z);
	  m_boundingBox.addToBBox(v);
	}
	else {
	  Vertex v(CosSinTable::getInstance().cos(i-90)*m_outerRadius + m_bottomCenter.m_x,
		   CosSinTable::getInstance().sin(i-90)*m_outerRadius + m_bottomCenter.m_y,
		   m_height + m_bottomCenter.m_z);
    ...
    glPushMatrix();
      ...
      ColorSetter::getInstance().setColor(m_bottomCenter.m_z);
      glRotatef(180,0,1,0);
      gluPartialDisk(m_quadric, m_innerRadius, m_outerRadius, slices, loops, 360-m_sweepAngle-m_startAngle, m_sweepAngle);
    glPopMatrix();
    glPushMatrix();
      //Bottom
      ColorSetter::getInstance().setColor(m_bottomCenter.m_z + m_height);
      glTranslatef(0.,0.,m_height);
      gluPartialDisk(m_quadric, m_innerRadius, m_outerRadius, slices, loops, m_startAngle, m_sweepAngle);
    glPopMatrix();
  glPopMatrix();
  glEndList();
  ...
  // Add candidates to bounding box.
  //glTranslatef(m_bottomCenter.m_x + , m_bottomCenter.m_y + , m_bottomCenter.m_z + );
  //addToBBox(m_bottomCenter);

  //addToBBox(m_bottomCenter.m_x + xStartOuter,m_bottomCenter.m_y + yStartOuter,m_bottomCenter.m_z + m_height);
  //addToBBox(m_bottomCenter.m_x + xEndOuter,m_bottomCenter.m_y + yEndOuter, m_bottomCenter.m_z + 0);
  // addToBBox(m_bottomCenter.m_x + xStartInner, m_bottomCenter.m_y + yStartInner, m_bottomCenter.m_z + m_height);
  //addToBBox(m_bottomCenter.m_x + xEndInner, m_bottomCenter.m_y + yEndInner, m_bottomCenter.m_z + 0);
}
